...
def physio_node(state: AgentState):
    if state["safety_alert"]:
        plan = ["Seated Stretching", "Diaphragmatic Breathing", "Magnesium Glycinate"]
    else:
        plan = ["Standing Toe Raises", "Sit-to-Stands", "Tandem Stance Balance"]
    
    # Format for GitHub Table
    log_entry = f"| {datetime.now().strftime('%Y-%m-%d %H:%M')} | {state['fatigue_level']} | {state['carpet_trips']} | {', '.join(plan)} |"
